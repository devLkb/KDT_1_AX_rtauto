...
{
    /// <summary>
    /// 라이브 데모(Pipeline_Demo)에서 전체 화면 카메라와 그랩 클로즈업 카메라를 전환한다.
    /// CinemachineBrain은 Priority가 가장 높은 CinemachineVirtualCamera를 자동으로 활성화하므로,
    /// 전환은 선택된 카메라만 ActivePriority로 올리고 나머지는 InactivePriority로 내리는 방식으로 이뤄진다.
    /// </summary>
    public sealed class DemoCameraSwitcher : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            GUILayout.BeginArea(new Rect(Screen.width - 260, 10, 250, 75 + cameras.Length * 25), GUI.skin.box);
            GUILayout.Label("카메라");
            for (int i = 0; i < cameras.Length; i++)
            {
