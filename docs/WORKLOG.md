...
### §25-4. 남은 것 — 정지 미세떨림 = 이제 입력 노이즈 추종 문제

정지 구간 로봇 손끝 1s 이동폭 3.8cm(입력 노랑 2.6 → 스무딩후 2.6 → 로봇 3.8, ×1.5 증폭).
리밋사이클(병리)은 소멸 — 남은 건 비전 노이즈가 그대로 관절로 흐르는 **필터 튜닝**(Phase 4):
One Euro tip min_cutoff 하향, targetLerp가 저주파 드리프트는 못 거르는 점, 로봇 ×1.5 증폭
(추종 지연+체이스 오버슈트 의심) 순으로. 다음 세션 과제.
