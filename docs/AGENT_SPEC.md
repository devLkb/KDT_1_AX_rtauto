...
순서와 크기는 v2 계약 안에서 고정한다. 관절 위치는 URDF 전체 limit이 아니라 학습 안전범위로 정규화한다.
...
2. 받침대 workspace pose 재표본화 및 이동
3. 공을 받침대 상단에 배치하고 선속도·각속도 0, gravity on 복원
...
spec 버전을 올린다. v1 checkpoint는 v2에서 재사용하지 않으며 기존 결과 디렉터리는 보존한다.
