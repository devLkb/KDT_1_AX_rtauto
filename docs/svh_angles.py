...
각 채널의 (min_rad, max_rad)는 반드시 SVH URDF/매뉴얼 값으로
채워야 합니다. 지금 값은 자리표시(placeholder)입니다.
"""
import numpy as np
...
def compute_svh_angles(lm):
    ...
    return {
        ...
        "index_flexion":    _flexion(lm, INDEX),
        "middle_flexion":   _flexion(lm, MIDDLE),
        "ring_flexion":     _flexion(lm, RING),
        "pinky_flexion":    _flexion(lm, PINKY),
        # 아래 3개는 SVH 커플링/spread 채널. 필요에 맞게 조정.
        "finger_spread":    _spread(lm),
        "ring_pinky_couple": (_flexion(lm, RING) + _flexion(lm, PINKY)) / 2.0,
        "index_middle_couple": (_flexion(lm, INDEX) + _flexion(lm, MIDDLE)) / 2.0,
    }
...
#   -> 아직 placeholder! 2단계에서 SVH URDF 실제 값으로 교체 필수.
#      이 값 그대로 실물 SVH에 보내지 말 것.
# =========================================================================
SVH_CHANNELS = [
    # name,                human_min, human_max, svh_min, svh_max
    ("thumb_flexion",        0.10,  2.00,   0.0, 0.98),
    ("thumb_opposition",     0.10,  0.55,   0.0, 0.99),
    ("index_flexion",        0.15,  3.05,   0.0, 1.33),
    ("middle_flexion",       0.12,  3.15,   0.0, 1.33),
    ("ring_flexion",         0.12,  3.30,   0.0, 1.33),
    ("pinky_flexion",        0.15,  3.25,   0.0, 1.33),
    ("finger_spread",        0.30,  1.10,   0.0, 0.28),
    ("index_middle_couple",  0.15,  3.10,   0.0, 1.33),
    ("ring_pinky_couple",    0.15,  3.25,   0.0, 1.33),
]
