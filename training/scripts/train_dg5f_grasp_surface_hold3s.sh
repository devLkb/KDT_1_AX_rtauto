...
SOURCE_RUN_ID="${DG5F_HOLD_SOURCE_RUN_ID:-dg5f_vdi_floor_safe_transfer_gpu_20260722}"
RUN_ID="${RUN_ID:-dg5f_vdi_surface3cm_hold3s_transfer_gpu_20260722}"
RESULTS_DIR="${RESULTS_DIR:-$ROOT/training/results}"
CONFIG="${CONFIG:-$ROOT/training/config/dg5f_grasp_surface_hold3s_finetune.yaml}"
ENV_PATH="${ENV_PATH:-$ROOT/training/builds/DG5FGraspSurfaceHold3s-20260722/DG5FGrasp.x86_64}"
MODE="${1:-start}"
...
case "$MODE" in
  ...
  resume)
    [[ -f "$RESULTS_DIR/$RUN_ID/DG5FGrasp/checkpoint.pt" ]] || {
      echo "[ERROR] hold checkpoint not found: $RESULTS_DIR/$RUN_ID/DG5FGrasp/checkpoint.pt" >&2
      exit 2
    }
    extra_args=(--resume)
    ;;
