...
RUN_ID="${DG5F_RUN_ID:-dg5f_v2_stablegrasp_v3_lr5e5_gpu_fixed}"
CONFIG="${DG5F_CONFIG:-$ROOT/training/config/dg5f_grasp_v3.yaml}"
...
PLAYER="${ENV_PATH:-$ROOT/training/builds/DG5FGraspV3/DG5FGrasp.x86_64}"
TIMEOUT_SECONDS="${DG5F_EVAL_TIMEOUT_SECONDS:-600}"
TRAINER_PID_FILE="$RESULTS_DIR/$RUN_ID/run_logs/evaluation.trainer.pid"

[[ -f "$RESULTS_DIR/$RUN_ID/DG5FStableGrasp/checkpoint.pt" ]] || {
  echo "[ERROR] checkpoint가 없습니다: $RESULTS_DIR/$RUN_ID/DG5FStableGrasp/checkpoint.pt" >&2
  exit 2
}
[[ -x "$PLAYER" ]] || {
