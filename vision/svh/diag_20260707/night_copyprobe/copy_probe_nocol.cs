...
string[] killOrder = { "SvhHandDriver", "SvhJointLogger", "HandSliderUI", "SvhReceiver", "SvhSelfCollisionIgnore" };
int killed = 0;
...
// [변형 A] 관성·CoM을 현재 계산값으로 동결한 뒤 콜라이더 전부 제거
...
foreach (var col in copy.GetComponentsInChildren<Collider>(true).ToArray()) {
...
string csvPath = Path.Combine(outDir, "copy_nocol_log.csv");
string sumPath = Path.Combine(outDir, "copy_nocol_summary.txt");
...
return "SETUP OK (no-collider variant). killed=" + killed + " frozen=" + frozen + " collidersRemoved=" + colsKilled + " joints=" + bodies.Count;
