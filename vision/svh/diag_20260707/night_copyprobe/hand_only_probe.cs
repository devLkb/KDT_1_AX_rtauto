...
// svh_base_link 이하만 복제 -> 새 인스턴스는 부모 AB가 없어 루트 articulation이 됨 -> immovable.
...
foreach (var t in orig.GetComponentsInChildren<Transform>())
    if (t.name == "svh_base_link") { srcHand = t; break; }
if (srcHand == null) return "ERROR: svh_base_link not found";

foreach (var n in new[] { "ur5e_svh_COPY", "svh_HAND_COPY" }) {
...
string csvPath = Path.Combine(outDir, "hand_only_log.csv");
string sumPath = Path.Combine(outDir, "hand_only_summary.txt");
...
cb = () => {
    try {
        ...
        if (phase == 1 && t > 10.5f) {
            ...
            var g = GameObject.Find("svh_HAND_COPY");
...
return "SETUP OK (hand-only). bodies=" + hand.GetComponentsInChildren<ArticulationBody>().Length +
